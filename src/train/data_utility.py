...
class DataUtility:
    def __init__(self, vocab_file_in_words=None, vocab_file_in_letters=None, vocab_file_out=None,
        ...
        if vocab_file_in_words and vocab_file_in_letters and vocab_file_out:
            ...
            with open(vocab_file_out, mode="r") as f:
                for line in f:
                    token, id = line.split("##")
                    id = int(id)
                    if token.startswith(self.es_flag) and self.en_vocab_size_out < 0:
                        self.en_vocab_size_out = id
        ...
        for phrase, phrase_id in self.token2id_phrase.items():
            first_word_of_phrase = self.en_flag + phrase.split()[0]
            if first_word_of_phrase in self.token2id_out:
                head_id_of_phrase = self.token2id_out[first_word_of_phrase]
